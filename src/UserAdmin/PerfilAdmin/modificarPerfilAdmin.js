...
function ModificarPerfilComponente(){
    ...
    const [administrador, setadministrador] = useState(location.state && location.state.administrador);

    const [administradorModificado, setAdministradorModificado] = useState({documento:administrador.documento, mail:'', password:'', nombre:''})

    const manejarEntradaUsuarioModificado = (e) =>{
        setAdministradorModificado({...administradorModificado, [e.target.name]: e.target.value})
    }
    //TODO
    ...
    const modificarPerfil = async () => {
        try {
            console.log("ENTRO")
            const respuesta = await fetch(`http://localhost:8080/api/personas/${administrador.documento}`, {
              method: 'PUT',
              headers: {
